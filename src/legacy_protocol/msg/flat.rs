...
pub fn deserialize_span_data_index_map(map: &[u32]) -> SpanDataIndexMap {
    ...
    map.chunks_exact(5)
        .map(|span| {
            let &[file_id, ast_id, start, end, e] = span else {
                unreachable!()
            };
            Span {
                anchor: SpanAnchor {
...
impl<'a, S: std::fmt::Debug + InternableSpan> Writer<'a, '_, S> {
    ...
    /// DONE
    /// Assuming RA, inserts `span` into `self.span_data_table`,
    /// returning its index
    ...
    fn subtree(&mut self, idx: usize, subtree: tt::iter::TtIter<'a, S>) {
        ...
        for child in subtree {
            // idx_tag is a "segmented pointer" to one of the arrays
    ...
    /// DONE
    /// Inserts `text` into `self.string_table` and `self.text`,
    ///     with `self.string_table[text] == self.text.index_of(text)`
    /// `self.text` owns a clone of `text`, and `self.string_table` has the original reference,
    ///     but pointing to the separate owned value.
    /// TODO -- Why not IndexSet?
    ///      -- How do we get a stable order for serialization? Order-by-value?
